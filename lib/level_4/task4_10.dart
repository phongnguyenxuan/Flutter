...
class Task4_10 extends StatefulWidget {
  const Task4_10({super.key});

  @override
  State<Task4_10> createState() => _Task4_10State();
}

class _Task4_10State extends State<Task4_10> {
...
//can bang do dai 2 chuoi
    ...
    var input = res.trim().toLowerCase().split('');
    var list = input.toSet();
    var myMap = Map<String, int>.fromIterables(
        list, List.generate(list.length, (i) => 0));
    input.forEach((e) => myMap[e] = myMap[e]! + 1);

    setState(() {
      result = myMap.toString();
    });
  }
